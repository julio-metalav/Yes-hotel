...
function mapDbHospedeToInternal(row, fnrhRow, reservaRow) {
  ...
  const base = {
    ...
    isMinor: isMinor,
    guestRole: guestRole,
    responsibleGuestId: row.responsible_guest_id || null,
  };
  if (fnrhRow && fnrhRow.link_token && fnrhRow.id) {
    base.fnrhLink =
      "./fnrh-preenchimento.html?v=2&guest_id=" +
      encodeURIComponent(fnrhRow.id) +
      "&token=" +
      encodeURIComponent(fnrhRow.link_token);
...
function getProximaAcaoReserva(reserva) {
  ...
  if (enviados > 0) return "Aguardar conclusão da FNRH";
  if (hasFnrhPendente(reserva) && isPagamentoOk(reserva)) {
    return "Aguardar conclusão da FNRH";
  }
  ...
  if (acessoLiberadoEfetivo(reserva) && !reserva.entrouNoApto) {
    return "Aguardar chegada do hóspede";
  }
  if (isCheckinConcluido(reserva)) return "Check-in operacional concluído";
  return "";
}
...
function formatFnrhSituacaoLabel(reserva) {
  var confirmados = getFnrhConfirmadas(reserva);
  var totalH = getHospedesTotal(reserva);
  if (totalH <= 0) return "Pendente";
  if (confirmados >= totalH) return "Concluída";
  if (confirmados === 0) return totalH === 1 ? "Pendente" : "0 de " + totalH + " concluídas";
  return confirmados + " de " + totalH + " concluídas";
}

function formatAcessoSituacaoLabel(reserva) {
  if (reserva.entrouNoApto) {
...
function formatCentavosPainel(centavos) {
  var api = getPagarmePaymentUiApi();
  if (api && typeof api.formatCentavosToBRL === "function" && centavos != null) {
    return api.formatCentavosToBRL(centavos);
  }
  if (centavos == null || !Number.isFinite(Number(centavos))) return "";
  return "R$ " + (Number(centavos) / 100).toFixed(2).replace(".", ",");
}

/** Pagamento compacto dentro do card Situação — sem card Pagar.me separado. */
function buildPagamentoSituacaoHtml(reserva, fin) {
  var html =
    '<div class="detail-situacao-pagamento" data-fin-status="' +
    escapeHtml(fin.status) +
    '">';
  html +=
    '<p class="detail-situacao-valor">' + escapeHtml(fin.label) + "</p>";

  if (fin.status === "pendente_comissionado") {
    html +=
      '<p class="detail-situacao-sub">Regularizar pagamento no HITS</p></div>';
    return html;
  }
  ...
  var payUi = resolvePaymentUiForReserva(reserva);
  if (payUi && payUi.cobranca) {
    var cob = payUi.cobranca;
    var bits = [];
    if (cob.status) bits.push(String(cob.status));
    var valorFmt = formatCentavosPainel(cob.valor_centavos);
    if (valorFmt) bits.push(valorFmt);
    if (cob.metodo) bits.push(String(cob.metodo));
    if (bits.length) {
      html +=
        '<p class="detail-situacao-sub">' + escapeHtml(bits.join(" · ")) + "</p>";
    }
    if (payUi.ctaKind === "pagarme_ver" || payUi.ctaKind === "pagarme_revisao") {
      html +=
        '<button type="button" class="secondary-button detail-pagarme-inline-btn detail-recomendacao-cta-btn" data-recomendacao-cta="' +
        escapeHtml(payUi.ctaKind) +
        '">Ver cobrança</button>';
    }
  }
  html += "</div>";
...
/**
 * Central de ações do detalhe: FNRH e senha lado a lado, cada uma com o estado
 * real da etapa e a ação disponível. Só apresentação — ids, classes e data-*
 * dos botões são os que bindDetailListeners / executeRecomendacaoCta já tratam.
 */
function buildCentralAcoesHtml(reserva, ctx, rec, btns) {
  var ctaKind = rec && rec.cta && rec.cta.kind ? String(rec.cta.kind) : "";
  var fnrhCompleta = isFnrhCompleta(reserva);
  var enviados = ctx.enviadosCount || 0;
  var prontos = ctx.prontosCount || 0;
  var faltam = ctx.faltamCount || 0;
  var naoIdent = ctx.naoIdentCount || 0;

  var fnrhEstado = formatFnrhSituacaoLabel(reserva);
  var fnrhTom = fnrhCompleta ? "ok" : "pending";
  var fnrhSub = "";
  var fnrhAcoes = "";
  if (fnrhCompleta) {
    fnrhSub = "Ficha confirmada pelo hóspede. Nenhuma ação necessária.";
  } else if (btns.enviarLinksBtnHtml) {
    fnrhSub =
      enviados > 0
        ? "Link já enviado a " + enviados + " hóspede(s); " + prontos + " ainda sem link."
        : "Link ainda não enviado ao hóspede.";
    fnrhAcoes = btns.enviarLinksBtnHtml;
  } else if (btns.reenviarFnrhTopoBtnHtml) {
    fnrhSub = "Link enviado. Aguardando o hóspede preencher.";
    fnrhAcoes = btns.reenviarFnrhTopoBtnHtml;
  } else if (naoIdent > 0) {
    fnrhSub = "Complete o nome do(s) hóspede(s) no cartão abaixo.";
  } else if (faltam > 0) {
    fnrhSub = "Falta e-mail ou WhatsApp. Corrija o contato no cartão do hóspede.";
  } else if (enviados > 0) {
    fnrhSub = "Link enviado. Aguardando o hóspede preencher.";
  }

  var senhaJaEnviada = !!(reserva.senhaEnviadaEm || obterUltimosEventosSenha(reserva).lastOkSenha);
  var senhaGerada = acessoLiberadoEfetivo(reserva);
  var falha = detectarUltimaFalhaCredencial(reserva);
  var acesso = formatAcessoSituacaoLabel(reserva);
  var senhaEstado = "";
  var senhaTom = "pending";
  var senhaSub = "";
  if (reserva.entrouNoApto) {
    senhaEstado = "Hóspede no apartamento";
    senhaTom = "ok";
  } else if (senhaJaEnviada) {
    senhaEstado = "Enviada";
    senhaTom = acesso.accent === "error" ? "error" : "ok";
    var enviadaEm = reserva.senhaEnviadaEm
      ? formatHistoricoTimestamp(new Date(reserva.senhaEnviadaEm))
      : "";
    senhaSub =
      acesso.accent === "error"
        ? acesso.label
    ...
    senhaEstado = "Não gerada";
    if (isProntaParaLiberarAcesso(reserva)) {
      senhaSub = "Pagamento e FNRH em dia. Pode gerar a senha.";
    } else {
      var pend = [];
      if (!isPagamentoOk(reserva)) pend.push("pagamento");
      if (!fnrhCompleta) pend.push("FNRH");
      senhaSub = pend.length ? "Aguardando " + pend.join(" e ") + "." : "";
    }
  }
  var senhaAcoes = (btns.gerarSenhaBtnHtml || "") + (btns.enviarSenhaBtnHtml || "");

  var fnrhRec = ctaKind === "enviar_fnrh" || ctaKind === "reenviar_fnrh";
  var senhaRec = ctaKind === "gerar_senha" || ctaKind === "liberar_acesso";

  function item(key, titulo, estado, tom, sub, acoes, recomendada) {
    return (
      '<div class="detail-central-item detail-central-item--' +
      key +
      (recomendada ? " is-recomendada" : "") +
      '">' +
      '<p class="detail-central-item-title">' +
      titulo +
      "</p>" +
      '<p class="detail-central-item-estado is-' +
      tom +
      '">' +
      escapeHtml(estado) +
      "</p>" +
      (sub ? '<p class="detail-central-item-sub">' + escapeHtml(sub) + "</p>" : "") +
      (acoes ? '<div class="detail-central-item-acoes">' + acoes + "</div>" : "") +
      "</div>"
    );
  }

  return {
    temBotao: !!(fnrhAcoes || senhaAcoes),
    html:
      '<div class="detail-central-acoes" id="detail-central-acoes">' +
      '<p class="detail-acao-kicker">Central de ações</p>' +
      '<div class="detail-central-acoes-grid">' +
      item("fnrh", "FNRH", fnrhEstado, fnrhTom, fnrhSub, fnrhAcoes, fnrhRec) +
      item("senha", "Senha de acesso", senhaEstado, senhaTom, senhaSub, senhaAcoes, senhaRec) +
      "</div></div>",
  };
}

/** Topo do detalhe: situação única (pagamento/FNRH/acesso/próxima ação) + Central de ações. */
function buildSituacaoAcaoTopoHtml(
  ...
  var fnrhLabel = formatFnrhSituacaoLabel(reserva);
  var acessoInfo = formatAcessoSituacaoLabel(reserva);
  var proxOp = getProximaAcaoReserva(reserva);
  // Próxima ação operacional prevalece; financeiro só se a régua operacional não tiver texto.
  var nextActionLabel = proxOp || fin.nextAction || "";

  var rec = derivarRecomendacaoOperacional(reserva, ctx);
  var rid = escapeHtml(String(reserva.id));

  // FNRH, senha e liberação de acesso vivem na Central de ações; aqui só as
  // demais próximas ações (cobrança, entrada, demo).
  var kindsNaCentral = {
    enviar_fnrh: true,
    reenviar_fnrh: true,
    gerar_senha: true,
  ...
  var isComissionado = fin.status === "pendente_comissionado";

  if (!aguardarChegada && !isComissionado) {
    if (fin.status === "pendente" && payUiTopo && payUiTopo.ctaKind && payUiTopo.ctaLabel) {
      // Cobrança compacta já tem "Ver cobrança"; CTA de gerar fica na próxima ação.
      if (payUiTopo.ctaKind === "pagarme_cobrar" || payUiTopo.ctaKind === "pagarme_classificar") {
        primaryRow =
          '<button type="button" class="primary-button detail-recomendacao-cta-btn" data-recomendacao-cta="' +
    ...
    } else if (rec.cta && rec.cta.kind && !kindsNaCentral[rec.cta.kind] && rec.cta.label) {
      if (
        String(rec.cta.kind).indexOf("pagarme_") !== 0 ||
        (fin.status === "pendente" && !isComissionado)
      ) {
        primaryRow =
          '<button type="button" class="primary-button detail-recomendacao-cta-btn" data-recomendacao-cta="' +
          escapeHtml(rec.cta.kind) +
  ...
  var central = buildCentralAcoesHtml(reserva, ctx, rec, {
    enviarLinksBtnHtml: enviarLinksBtnHtml || "",
    reenviarFnrhTopoBtnHtml: reenviarFnrhTopoBtnHtml || "",
    enviarSenhaBtnHtml: enviarSenhaBtnHtml || "",
    gerarSenhaBtnHtml: gerarSenhaBtnHtml || "",
  });

  // Painel de contato (e-mail / WhatsApp) antes de enviar FNRH ou senha:
  // mesmo fluxo de openTopContatoPanel / submitDetailTopContatoPanel.
  ...
  if (primaryRow || nextActionLabel) {
    acaoBlock =
      ...
      (nextActionLabel
        ...
        : "") +
      (primaryRow ? '<div class="detail-top-actions">' + primaryRow + "</div>" : "") +
      "</div>";
  }
  acaoBlock += central.html + contatoPanelHtml;
  ...
  var acessoClass =
    acessoInfo.accent === "entrou"
      ? " detail-situacao-acesso--entrou"
      : acessoInfo.accent === "ok"
  ...
  return (
    ...
    buildPagamentoSituacaoHtml(reserva, fin) +
    "</dd></div>" +
    "<div><dt>FNRH</dt><dd>" +
...
function buildOrigemComercialHtml(reserva) {
  ...
  function wrapOrigem(inner) {
    if (!inner) return "";
    return (
      '<div class="reservation-detail-section reservation-detail-origem">' +
      '<p class="reservation-detail-section-title">Origem da reserva</p>' +
      '<dl class="detail-situacao-grid">' +
      inner +
      "</dl></div>"
    );
  ...
  var isBookingEngine =
    (finApi && typeof finApi.isBookingEngineChannel === "function"
      ? finApi.isBookingEngineChannel(channel) || finApi.isBookingEngineChannel(billing)
      : /^booking\s*engine$/i.test(channel)) || false;

  var isMotor =
    (finApi && typeof finApi.isParticularMotorReservation === "function"
      ? finApi.isParticularMotorReservation({
          channelManager: manager,
          salesChannel: channel,
          billingEntity: billing,
        }) && !isBookingEngine
      : false) ||
    /^motor\s+de\s+reservas$/i.test(channel) ||
    /^motor\s+de\s+reservas$/i.test(billing);
  ...
  if (isManualHits && finApi && typeof finApi.isManualHitsDirectReservation === "function") {
    isManualHits = finApi.isManualHitsDirectReservation({
      ...
      salesChannel: channel,
      billingEntity: billing,
      reservationChannelId: reserva.reservationChannelId,
    });
  ...
  if (isManualHits) {
    return wrapOrigem("<div><dt>Origem</dt><dd>Reserva direta</dd></div>");
  }
  ...
  if (isBookingEngine) {
    ...
    return wrapOrigem(linesBe.join(""));
  }

  if (isMotor && !manager) {
    return wrapOrigem(
      "<div><dt>Origem</dt><dd>" +
        escapeHtml(channel || billing || "Motor de Reservas") +
        "</dd></div>",
    );
  }
...
function buildComunicacoesCompactHtml(reserva) {
  var envios = Array.isArray(reserva.comunicacaoEnviosOperacional)
    ? reserva.comunicacaoEnviosOperacional
    : [];
  if (!envios.length) return "";
  var lines = [];
  var seen = {};
  for (var i = 0; i < envios.length && lines.length < 4; i++) {
    var e = envios[i];
    if (!e) continue;
    var canal = labelCanalComunicacaoPainel(e.canal);
    var prop = labelPropositoComunicacao(e.proposito);
    var when = e.createdAt ? formatIsoOperacional(e.createdAt) : "";
    var status = labelStatusEnvioComunicacao(e.status);
    var key = canal + "|" + prop + "|" + when;
    if (seen[key]) continue;
    seen[key] = true;
    var parts = [canal, prop];
    if (status && status !== "—") parts.push(status);
    if (when) {
      var hm = when.match(/\d{1,2}:\d{2}/);
      parts.push(hm ? hm[0] : when);
    }
    lines.push("<li>" + escapeHtml(parts.filter(Boolean).join(" · ")) + "</li>");
  }
  if (!lines.length) return "";
  return (
    '<div class="reservation-detail-section reservation-detail-comms-compact">' +
    '<p class="reservation-detail-section-title">Comunicações</p>' +
    '<ul class="detail-comms-list">' +
    lines.join("") +
    "</ul></div>"
  );
}
...
function renderDetail(reserva) {
  ...
  // Composição vem do PMS/HITS — painel nunca adiciona/remove/redefine principal.
  const compositionReadonly = true;
  const guestMgmt = {
    mode: "readonly",
    label: "",
    showHeaderAdd: false,
  ...
  const responsibleNameById = {};
  hospedes.forEach(function (g) {
    if (g && g.id) responsibleNameById[String(g.id)] = (g.nome || "").trim();
  });
  const guestsHtml = hospedes
    .map((h, index) => {
      ...
      const isMinor = h.isMinor === true || h.guestRole === "minor";
      const roleBits = [];
      roleBits.push(h.principal ? "Principal" : "Acompanhante");
      if (isMinor) roleBits.push("Menor");
      else if (h.guestRole === "primary_adult" || h.guestRole === "adult_companion" || h.isMinor === false) {
        roleBits.push("Adulto");
      } else if (h.guestRole || h.isMinor != null) {
        roleBits.push("Adulto");
      }
      const fnrhLine = confirmed
        ? "FNRH concluída"
        : h.fnrhStatus === "confirmado_hospede" || h.fnrhStatus === "confirmado_hotel"
          ? "FNRH concluída"
          : "FNRH pendente";
      const responsibleLine =
        isMinor && h.responsibleGuestId
          ? '<p class="guest-detail-responsavel">Responsável: ' +
            escapeHtml(responsibleNameById[String(h.responsibleGuestId)] || "adulto responsável") +
            "</p>"
          : "";
      ...
      const contactBlock = isMinor
        ? `<dl class="guest-detail-readout guest-detail-readout--compact">
            <div><dt>Telefone</dt><dd>${waDisplay || "—"}</dd></div>
        ...
        : `<dl class="guest-detail-readout guest-detail-readout--compact">
          ...
          <div class="guest-detail-edit-fields hidden" data-guest-edit="${index}">
      ...
      return {
        cardHtml: `
        <div class="guest-detail-card${confirmed ? " guest-detail-card--ok" : ""}${isMinor ? " guest-detail-card--minor" : ""}" data-guest-index="${index}">
          <p class="guest-detail-nome">${nomeVal || "—"}</p>
          <p class="guest-detail-role">${escapeHtml(roleBits.join(" · "))}</p>
  ...
  const manageBlockHtml = "";
  void compositionReadonly;
  ...
  // Central de ações: a ação recomendada ganha destaque (primary); as demais
  // ficam visíveis como secundárias. Só muda a classe visual do botão.
  const ctxRecomendacao = buildRecomendacaoOperacionalCtx(reserva);
  const recomendacaoTopo = derivarRecomendacaoOperacional(reserva, ctxRecomendacao);
  const ctaRecomendada =
    recomendacaoTopo && recomendacaoTopo.cta && recomendacaoTopo.cta.kind
      ? String(recomendacaoTopo.cta.kind)
      : "";
  const classeBotaoCentral = (kind) =>
    ctaRecomendada === kind ? "primary-button" : "secondary-button";
  ...
  if (naoIdentCount > 0) {
    ...
    enviarLinksBtnHtml = `<button type="button" class="${classeBotaoCentral("enviar_fnrh")} detail-top-acao-btn detail-enviar-links-btn" id="detail-enviar-links-btn" data-reserva-id="${escapeHtml(reserva.id)}" title="${escapeHtml(getEnviarButtonLabel())}">${escapeHtml(getEnviarLinkTopoLabel())}</button>`;
  ...
  // Rótulo pelo estado real da senha (gerar / enviar / reenviar). O handler
  // (data-acao-credencial) e a política de credenciais seguem iguais.
  const labelCredenciais = senhaJaEnviada
    ? "Reenviar senha"
    : acessoLiberadoEfetivo(reserva)
      ? "Enviar senha"
  ...
  const enviarSenhaBtnHtml = temBotaoSenhaBackend
    ? `<button type="button" class="${classeBotaoCentral("gerar_senha")} detail-top-acao-btn detail-enviar-senha-btn" id="detail-enviar-senha-btn" data-reserva-id="${escapeHtml(reserva.id)}" data-acao-credencial="${senhaJaEnviada ? "reenviar" : "gerar_enviar"}">${escapeHtml(labelCredenciais)}</button>` +
      (senhaJaEnviada
  ...
  if (podeReenviarFnrhTopo) {
    reenviarFnrhTopoBtnHtml = `<button type="button" class="${classeBotaoCentral("reenviar_fnrh")} detail-top-acao-btn detail-reenviar-fnrh-topo-btn" id="detail-reenviar-fnrh-topo-btn" data-reserva-id="${escapeHtml(reserva.id)}">Reenviar link FNRH</button>`;
  }

  // "Gerar senha" = liberar acesso (mesmo handler do CTA liberar_acesso):
  // cria a credencial e provisiona no TTLock sem enviar nada ao hóspede.
  let gerarSenhaBtnHtml = "";
  if (isProntaParaLiberarAcesso(reserva)) {
    gerarSenhaBtnHtml = `<button type="button" class="${classeBotaoCentral("liberar_acesso")} detail-top-acao-btn detail-recomendacao-cta-btn detail-gerar-senha-btn" id="detail-gerar-senha-btn" data-reserva-id="${escapeHtml(reserva.id)}" data-recomendacao-cta="liberar_acesso">Gerar senha</button>`;
  }
  ...
  detailBodyElement.innerHTML = `
    ...
    ${localModeDetailHtml}
    ${ttlockSectionHtml}
    ${buildComunicacoesCompactHtml(reserva)}
