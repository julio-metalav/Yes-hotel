...
function normalizeApartmentCode(roomCode: string | undefined): string {
  if (!roomCode) {
    throw new Error("Resposta HITS sem rooms[].code utilizavel.");
  }
...
function resolvePrimaryRoom(detail: HitsReservationDetail): HitsReservationRoom {
  const rooms = detail.rooms ?? [];

  if (rooms.length === 0) {
    throw new Error("Resposta HITS sem rooms[] para mapear a reserva interna.");
  }

  // Fallback conservador enquanto nao houver criterio mais forte confirmado.
  return rooms[0];
}

function resolvePrimaryGuest(
  detail: HitsReservationDetail,
  primaryRoom: HitsReservationRoom,
): HitsReservationGuest | undefined {
...
function resolveGuestMainName(
  detail: HitsReservationDetail,
  guest: HitsReservationGuest | undefined,
): string {
...
export function mapHitsReservationDetailToInternalReservation(
  detail: HitsReservationDetail,
): InternalReservation {
  ...
  if (!reservationId) {
    throw new Error("Resposta HITS sem idReservation utilizavel.");
  }

  if (!checkIn || !checkOut) {
    throw new Error(
      `Resposta HITS sem checkIn/checkOut utilizavel para a reserva ${reservationId}.`,
    );
  }
  ...
  return {
    ...
    checkIn,
    checkOut,
    status: mapExternalStatusToInternalStatus(primaryRoom.status),
    updatedAt,
  };
}
