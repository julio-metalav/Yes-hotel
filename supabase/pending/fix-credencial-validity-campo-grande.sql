-- Correção da janela de validade ao criar credencial no liberar acesso.
-- Projeto: minmmecajnmjqlgacfoz
--
-- Altera SOMENTE o cálculo de valido_de / valido_ate na função do trigger.
-- Não mexe em reservas, senhas, remote_keyboard_pwd_id, status nem credenciais existentes.
--
-- Causa: `(ts)::timestamptz` usava TimeZone da sessão (UTC) → 13:00Z / 11:00Z.
-- Correção: interpretar horário civil em America/Campo_Grande.
--   13:00 CG → 17:00Z | 11:00 CG → 15:00Z
...
begin
  ...
  -- Janela civil do hotel: check-in 13:00 / check-out 11:00 em America/Campo_Grande
