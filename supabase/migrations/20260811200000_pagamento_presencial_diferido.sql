-- Yes Hotel — pagamento presencial diferido (autorização operacional).
-- NÃO marca pago. NÃO altera Pagar.me. Fail-closed na feature (env/UI).
-- Fonte financeira vigente permanece: operacional_reservas.pagamento_status.

alter table public.operacional_reservas
  add column if not exists pagamento_presencial_diferido_autorizado boolean not null default false;

alter table public.operacional_reservas
  add column if not exists pagamento_presencial_diferido_autorizado_em timestamptz;

alter table public.operacional_reservas
  add column if not exists pagamento_presencial_diferido_autorizado_por uuid;

alter table public.operacional_reservas
  add column if not exists pagamento_presencial_diferido_autorizado_por_email text;

alter table public.operacional_reservas
  add column if not exists pagamento_presencial_diferido_efetivado boolean not null default false;

alter table public.operacional_reservas
  add column if not exists pagamento_presencial_diferido_efetivado_em timestamptz;

alter table public.operacional_reservas
  add column if not exists pagamento_presencial_diferido_regra text;

alter table public.operacional_reservas
  add column if not exists pagamento_presencial_diferido_primeiro_acesso_em timestamptz;

alter table public.operacional_reservas
  add column if not exists pagamento_presencial_diferido_deadline_em timestamptz;

alter table public.operacional_reservas
  add column if not exists pagamento_presencial_diferido_bloqueado_em timestamptz;

alter table public.operacional_reservas
  add column if not exists pagamento_presencial_diferido_bloqueado_motivo text;

alter table public.operacional_reservas
  add column if not exists pagamento_presencial_diferido_regularizado_em timestamptz;

alter table public.operacional_reservas
  drop constraint if exists operacional_reservas_ppd_regra_check;

alter table public.operacional_reservas
  add constraint operacional_reservas_ppd_regra_check
  check (
    pagamento_presencial_diferido_regra is null
    or pagamento_presencial_diferido_regra in ('util_19h', 'fim_semana_feriado_15h')
  );

comment on column public.operacional_reservas.pagamento_presencial_diferido_autorizado is
  'Autorização operacional para pagamento presencial diferido. NÃO significa pago.';

comment on column public.operacional_reservas.pagamento_presencial_diferido_efetivado is
  'Exceção efetivada no 1º acesso (janela 19h/15h). Prazo = 09:00 do dia seguinte.';

alter table public.operacional_acesso_tolerancias
  add column if not exists grace_mode text not null default 'standard_1h';

alter table public.operacional_acesso_tolerancias
  drop constraint if exists operacional_acesso_tolerancias_grace_mode_check;

alter table public.operacional_acesso_tolerancias
  add constraint operacional_acesso_tolerancias_grace_mode_check
  check (grace_mode in ('standard_1h', 'presencial_diferido_09h'));

comment on column public.operacional_acesso_tolerancias.grace_mode is
  'standard_1h = tolerância padrão; presencial_diferido_09h = prazo até 09:00 do dia seguinte.';

-- ---------------------------------------------------------------------------
-- RPC: permitir deadline ≠ 1h quando pending_snapshot contém
-- pagamento_presencial_diferido; gravar grace_mode.
-- Assinatura inalterada (mesmo overload / grants).
-- ---------------------------------------------------------------------------
...
comment on function public.yes_hotel_process_first_room_access is
  'PR5 + PPD: persistência atômica do primeiro acesso. Aceita deadline 09h quando snapshot contém pagamento_presencial_diferido. Service role only.';
